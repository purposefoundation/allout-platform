...
describe SpreedlyClient do
  ...
  describe '#gateway_token' do
    before :each do
      ENV.stub(:[]).with('SPREEDLY_501C3_ENV_KEY').and_return('ENV_KEY')
      ENV.stub(:[]).with('SPREEDLY_501C4_ENV_KEY').and_return('ENV_KEY')
      ENV.stub(:[]).with('SPREEDLY_501C3_APP_ACCESS_SECRET').and_return('ACCESS_TOKEN')
      ENV.stub(:[]).with('SPREEDLY_501C4_APP_ACCESS_SECRET').and_return('ACCESS_TOKEN')
    end

    it 'determines proper token from the environment' do
      ENV.stub(:[]).with('SPREEDLY_501C3_GATEWAY_USD').and_return('abracadabra')

      expect(spreedly_client.gateway_token('usd')).to eq 'abracadabra'
    end

    it 'handles different currencies' do
      ENV.stub(:[]).with('SPREEDLY_501C3_GATEWAY_CAD').and_return('canadian_token')
      ENV.stub(:[]).with('SPREEDLY_501C3_GATEWAY_EUR').and_return('european_token')

      expect(spreedly_client.gateway_token('cad')).to eq 'canadian_token'
      expect(spreedly_client.gateway_token('eur')).to eq 'european_token'
    end

    it 'handles different classifications' do
      spreedly_client = SpreedlyClient.new '501-c-4'
      ENV.stub(:[]).with('SPREEDLY_501C4_GATEWAY_CAD').and_return('canadian_token')

      expect(spreedly_client.gateway_token('cad')).to eq 'canadian_token'
    end

    it 'recovers from a non-existent environment' do
      ENV.stub(:[]).with('SPREEDLY_501C3_GATEWAY_USD').and_return(nil)

      expect(spreedly_client.gateway_token('usd')).to eq nil
    end

    it 'recovers from a non-existent classification' do
      spreedly_client = SpreedlyClient.new '501c4'
      ENV.stub(:[]).with('SPREEDLY_501C4_GATEWAY_USD').and_return(nil)
  ...
  describe "#purchase_and_hash_response" do
    ...
    it "should return purchase as a hash with the payment_method" do
      @spreedly.stub(:purchase_on_gateway) {
        Spreedly::Transaction.new_from(Nokogiri::XML(successful_purchase_response))
      }
    ...
    it "should return a hash with an error code, message, and payment method info on failure to purchase" do
      @spreedly.stub(:purchase_on_gateway).and_raise \
        Spreedly::XmlErrorsList.new(Nokogiri::XML(failed_purchase_response))
      purchase = spreedly_client.purchase_and_hash_response(payment_method)
