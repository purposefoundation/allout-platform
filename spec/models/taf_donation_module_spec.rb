...
describe TafDonationModule do

  def validated_donation_module(attrs)
    default_attrs = {active: 'true'}
    etm = FactoryGirl.create(:taf_donation_module, :default_currency => 'usd', :suggested_amounts => {'usd' => '1,2,3'}, :default_amount => {'usd' => '1'}, :donations_goal => 10, :thermometer_threshold => 0)
  ...
  it "should create sensible defaults" do
    TafDonationModule::AVAILABLE_CURRENCIES = { :usd => Money::Currency.new('USD') }
    dm = TafDonationModule.create
  ...
  it "should remove whitespace from suggested_amounts" do
    dm = FactoryGirl.create(:taf_donation_module, :suggested_amounts => {'usd' => '1 , 2,3'}, :default_amount => {'usd' => '1'}, :donations_goal => 10, :thermometer_threshold => 0)
    dm.suggested_amounts['usd'].should == '1,2,3'
  end

  it "should remove whitespace from recurring_suggested_amounts" do
    dm = FactoryGirl.create(:taf_donation_module, :recurring_suggested_amounts => {'monthly' => {'usd' => '1 , 2, 3 '}}, :default_amount => {'usd' => '1'}, :donations_goal => 10, :thermometer_threshold => 0)
  ...
  it "should remove whitespace from the suggested amounts string before saving" do
    dm = FactoryGirl.build(:taf_donation_module, :suggested_amounts => {'usd' => '1,   2 ,   3  '}, :default_amount => {'usd' => '1'})
    dm.save!
  ...
  describe "frequency options" do
    it "knows if it only allows one-off payments" do
      dm = FactoryGirl.create(:taf_donation_module, :frequency_options => {'one_off' => 'default', 'weekly' => 'hidden'})
    ...
    it "constructs a list of available frequencies suitable for use as dropdown options" do
      dm = FactoryGirl.create(:taf_donation_module, :frequency_options => {'one_off' => 'default', 'weekly' => 'hidden', 'monthly' => 'optional'})
      dm.available_frequencies_for_select.should == [ ['Donate Once', 'one_off'], ['Donate Monthly', 'monthly'] ]
    end

    it "should set default frequency" do
      dm = FactoryGirl.create(:taf_donation_module, :frequency_options => {'one_off' => 'default', 'monthly' => 'optional'})
    ...
    it "should set frequency option to optional when un-defaulting" do
      ...
      dm = FactoryGirl.create(:taf_donation_module, :frequency_options => {'one_off' => 'default', 'monthly' => 'optional', 'weekly' => 'hidden'})
      dm.default_frequency = :monthly
      dm.default_frequency.should == :monthly
  ...
  describe "as json" do
    before do
      TafDonationModule::AVAILABLE_CURRENCIES = {
    ...
    it "should include the number of members who have donated to that action page" do
      ...
      donation_module = FactoryGirl.create(:taf_donation_module, :pages => [page])
    ...
    it "should incude the donation classification" do
      old_donation_module = FactoryGirl.create(:taf_donation_module)
      old_donation_module.as_json[:classification].should eql "501(c)3"

      tax_deductible_donation_module = FactoryGirl.create(:tax_deductible_taf_donation_module)
      tax_deductible_donation_module.as_json[:classification].should eql "501(c)3"

      non_tax_deductible_donation_module = FactoryGirl.create(:non_tax_deductible_taf_donation_module)
      non_tax_deductible_donation_module.as_json[:classification].should eql "501(c)4"
    end

  end

  it_should_behave_like "content module with disabled content", :taf_donation_module
