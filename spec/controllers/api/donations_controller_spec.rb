...
describe Api::DonationsController do
  ...
  describe 'handle_failed_payment' do
    ...
    it "should get donation and member information and deliver mail" do
      ...
      action_page = OpenStruct.new
      action_page.slug = "donatenow"
      action_page.id = 1

      params = {}
      params[:action_page] = action_page.slug
      ...
      @movement.should_receive(:find_published_page).with("#{params[:action_page]}").and_return(action_page)

      Movement.should_receive(:find).and_return(@movement)
      Movement.should_receive(:find).and_return(@movement)
      Page.should_receive(:find).and_return(action_page)
